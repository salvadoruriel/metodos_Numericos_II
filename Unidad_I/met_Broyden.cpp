...
vex.change_val(0,0,-0.2); /*<** 1.vector inicial(cercano a la raiz) */
vex.change_val(1,0,-0.1);
double vx,vy;
int iter{5};
...
vy=vex.get_val(1,0);
/*<** 2.Jacob y F(x) */
Jacob.change_val(0,0, 8*vx -4 );
Jacob.change_val(1,0, sin(vx +1) );
Jacob.change_val(0,1, -6*vy);
Jacob.change_val(1,1, 2 );
pasF.change_val(0,0,4*vx*vx -3*vy*vy -4*vx -1);
pasF.change_val(1,0,2*vy -cos(vx+1) +1);
...
Akpas = Jacob.inversa();
    for(int i{2}; i<= iter; i++){
        ...
        vecF.change_val(0,0,4*vx*vx -3*vy*vy -4*vx -1);
        vecF.change_val(1,0,2*vy -cos(vx+1) +1);
